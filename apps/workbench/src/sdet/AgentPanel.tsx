...
import { generateRun, GenerateRunResult, ExecuteResult } from "./agenticApi";
...
export default function AgentPanel({
  ...
  // When true, the stream auto-scrolls to the newest content. The user can
  // pause it by scrolling up; it resumes once they scroll back to the bottom.
  const stickToBottomRef = useRef(true);
  const handleStreamScroll = useCallback(() => {
    const el = opencodeLiveRef.current;
    if (!el) return;
  ...
  // Auto agentic verification (generate -> execute -> heal) after generation.
  const [autoRun, setAutoRun] = useState(true);
  const [verifyHeadless, setVerifyHeadless] = useState(true);
  const [verifyRunning, setVerifyRunning] = useState(false);
  const [verifyError, setVerifyError] = useState<string | null>(null);
  const [verifyResult, setVerifyResult] = useState<GenerateRunResult | null>(null);
  const autoRunRef = useRef(autoRun);
  const runVerificationRef = useRef<() => void>(() => {});
  ...
  const runVerification = useCallback(async () => {
    ...
    const scenario = (lastUser?.content || "").trim() || `Generate and verify a test for ${url}`;
    setVerifyRunning(true);
    setVerifyError(null);
    setVerifyResult(null);
    try {
      const data = await generateRun(apiBase, {
        scenario,
        url,
        // The execute goal must be ACTION-oriented (not the generation prompt),
        // otherwise the planner "opens the URL and does nothing".
        goal: `Perform the end-to-end user journey for this feature on ${url} and reach a successful end state (e.g., form submitted / confirmation shown). Scenario context:\n${scenario}`,
        assertions: [],
        repo_dir: repoDir,
        test_path: "tests/e2e/session_test.spec.ts",
        // Reuse the spec already generated by the N0-N14 opencode session so the
        // loop always has a real file to heal/verify (bypasses the flaky generator).
  ...
  }, [apiBase, url, repoDir, messages, verifyHeadless, testCode]);

  useEffect(() => { autoRunRef.current = autoRun; }, [autoRun]);
  useEffect(() => { runVerificationRef.current = runVerification; }, [runVerification]);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === "opencode_event") {
          ...
          // Auto-run agentic verification (generate -> execute -> heal) when a
          // test was actually generated and the user has the toggle enabled.
          if (autoRunRef.current && data.test_code) {
  ...
  // Auto-scroll stream body when new events or code arrive, but only while the
  // user is pinned to the bottom. If they scroll up to read, we pause; scrolling
  // back to the bottom (handleStreamScroll) re-enables it.
  useEffect(() => {
    if (stickToBottomRef.current && opencodeLiveRef.current) {
      opencodeLiveRef.current.scrollTop = opencodeLiveRef.current.scrollHeight;
    }
  }, [opencodeLiveCode, opencodeEvents]);
  ...
  return (
    <div className="ap-panel">
      ...
      <div className="ap-repo-row ap-autorun-row">
        <label className="ap-autorun-label">
          <input
            type="checkbox"
            checked={autoRun}
            onChange={(e) => setAutoRun(e.target.checked)}
          />
          <span>Auto-verify after generation</span>
        </label>
        <label className="ap-autorun-label">
          <input
            type="checkbox"
            checked={verifyHeadless}
            onChange={(e) => setVerifyHeadless(e.target.checked)}
          />
          <span>Headless (uncheck to watch the browser)</span>
        </label>
        <button
          className="ap-btn"
          onClick={runVerification}
          disabled={verifyRunning || !url}
        >
          {verifyRunning ? "Verifying..." : "Verify Now"}
        </button>
      </div>
      ...
      {(opencodeRunning || opencodeEvents.length > 0 || opencodeLiveCode) && (
        <div className="ap-opencode-stream">
          ...
          <div
            className="ap-opencode-stream-body"
            ref={opencodeLiveRef}
      ...
      {verifyRunning && (
        <div className="agentic-result">
          <span className="badge">RUNNING agentic verification (generate → execute → heal)...</span>
        </div>
      )}
      {verifyError && <div className="error-bar">{verifyError}</div>}
      {verifyResult && (
        <div className="agentic-result">
          <div className="panel-header"><span>Agentic Verification</span></div>
          {verifyResult.attempts && verifyResult.attempts.length > 0 && (
            <div className="attempts-count">attempts: {verifyResult.attempts.length}</div>
          )}
          {verifyResult.generate_error && (
            <div className="error-bar">generate: {verifyResult.generate_error}</div>
          )}
          {verifyResult.execute && <ExecuteSummary ex={verifyResult.execute} />}
          {verifyResult.heal && (
            <>
              <div className="panel-header heal-header"><span>Self-Heal</span></div>
              <div className={`badge ${verifyResult.heal.success ? "ok" : "fail"}`}>
                {verifyResult.heal.success ? "HEALED" : "HEAL FAILED"}
              </div>
              {verifyResult.heal.error && <div className="error-bar">{verifyResult.heal.error}</div>}
              {verifyResult.heal.changed_locators && verifyResult.heal.changed_locators.length > 0 && (
                <div className="changed">Changed: {verifyResult.heal.changed_locators.join(", ")}</div>
              )}
              {verifyResult.heal.healed_code && (
                <pre className="code-block">{verifyResult.heal.healed_code}</pre>
              )}
            </>
          )}
        </div>
...
function ExecuteSummary({ ex }: { ex: ExecuteResult }) {
  const goalReached = ex.trace?.goal_reached ?? ex.goal_reached;
  return (
    <>
      <div className={`badge ${ex.success ? "ok" : "fail"}`}>
        {ex.success ? "SUCCESS" : "FAILED"}
        {goalReached !== undefined && ` · goal reached: ${goalReached ? "yes" : "no"}`}
      </div>
      {ex.error && <div className="error-bar">{ex.error}</div>}
      {ex.trace?.assertions && ex.trace.assertions.length > 0 && (
        <div className="assertions">
          <h4>Assertions</h4>
          {ex.trace.assertions.map((a, i) => (
      ...
      {ex.trace?.steps && ex.trace.steps.length > 0 && (
        <details className="steps">
          <summary>{ex.trace.steps.length} steps</summary>
          {ex.trace.steps.map((s) => (
